...
{
    public function __construct(private readonly ?CMSApplication $application = null)
    {
    }

    public function dispatch(OperationDefinition $operation, OperationInput $input): InternalApiResponse
    {
        $parent = $this->application ?? Factory::getApplication();

        if (!$parent instanceof CMSApplication || !$parent->isClient('api')) {
